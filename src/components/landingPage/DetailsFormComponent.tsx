...
const DetailsFormComponent = () => {
  const [selectButton, setSelectButton] = useState<EFormButton>(EFormButton.CAR_WORTH);
  return (
    <div className='mx-24 py-8'>
      <div className='p-24 bg-white rounded-2xl shadow-md w-full flex justify-center items-center'>
        <div className='w-3/4 h-full flex flex-col items-center'>
          ...
          <div className='flex gap-8 justify-center py-4'>
              ...
              } onClick={() => setSelectButton(EFormButton.CAR_WORTH)}>Car worth</div>
              <div className={
                cn(
                  'border rounded-full p-4 px-6 text-xl hover:bg-black hover:text-white transition cursor-pointer',
              ...
              onClick={() => setSelectButton(EFormButton.USED_CAR)}>Userd car</div>
              <div className={
                cn(
                  'border rounded-full p-4 px-6 text-xl hover:bg-black hover:text-white transition cursor-pointer',
                  selectButton === EFormButton.CAR_HISTORY && 'bg-black text-white',
                )
              } onClick={() => setSelectButton(EFormButton.CAR_HISTORY)}>Car history</div>
          </div>
          {selectButton === EFormButton.CAR_WORTH && (
            <CarWorthFormComponent />
          )}
          {selectButton === EFormButton.USED_CAR && (
            <UsedCarFormComponent />
          )}
